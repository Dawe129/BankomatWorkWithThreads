    ...
    while True:
        ...
        if req == "deposit":
            ...
            if target is None:
                ...
                print(" >", note)
                history_item = {"time": time.strftime("%Y-%m-%d %H:%M:%S"), 
                               "action": "incoming_transfer", 
                               "amount": amount,
                               "note": f"Příchozí převod z účtu {acc['account_id']}"}
        ...
        if req in ("deposit", "withdraw", "balance", "transfer"):
            ...
            history_item = {"time": ts, "action": req, "amount": amount, "note": note}
            acc['history'].append(history_item)

        if suspicious and req in ("withdraw", "deposit", "transfer"):
            print(" ! POZOR: Transakce označena jako PODEZŘELÁ (vysoká částka)!")
            ts = time.strftime("%Y-%m-%d %H:%M:%S")
            history_item = {"time": ts, "action": "alert", "amount": amount, 
                           "note": "Podezřelá transakce – vysoká částka"}
            acc['history'].append(history_item)
